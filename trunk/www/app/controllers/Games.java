 ...
 */
 public static void playGame(long gameId) {
	 ...
	 if (round == null) {
		 round = new Round(Round.find("byGameAndTask", game,
				 game.currentTask).fetch().size()).save();
		 round.game = game;
 ...
 */
 public static void nextRound(long gameId, double estimate, boolean playAgain)
 {
	 ...
	 Round round = new Round(Round.find("byGameAndStory", game, game.currentTask).fetch().size()).save();
	 round.game = game;
	 round.task = game.currentTask;
	 round.save();
	 playGame(gameId);
