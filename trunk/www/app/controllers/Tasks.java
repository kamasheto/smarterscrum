...
public class Tasks extends SmartCRUD
{
	...
	public static void getReport( long id )
	{
		...
		if( !temp.isEmpty() )
			maxdate.setTime( temp.get( temp.size() - 1 ).timestamp + (3 * 86400000) );
		String maxDate = maxdate.toString().substring( 0, 10 );

		Date mindate = new Date();
		if( !temp.isEmpty() )
			mindate.setTime( temp.get( 0 ).timestamp - (3 * 86400000) );
	...
	public static void changeTaskDeadline( long id, long newDeadline )
	{
		...
		task.save();
		Update.update( Security.getConnected(), "reload('tasks','task-" + task.id + "');" );
		renderJSON( true );
