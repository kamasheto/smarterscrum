	...
	public void doJob()
	{
		if( User.count() == 0 )
		{
			...
			for( int l = 0; l < 120; l++ )
			{
				...
				// new Log(users.get(0), "TEST", "TEST", 0, projects.get(0), new
				// Date()).save();
				else
					Log.addLog( "Test", users.get( 1 ), projects.get( 1 ) );
				// new Log(users.get(1), "TEST", "TEST", 0, projects.get(1), new
				// Date()).save();
			}
			// ************ LOGS END ************
			...
			for( int i = 0; i < 10; i++ )
			{
				...
				if( i < 4 )
				{
					ma = new MeetingAttendance( users.get( i ), users.get( i ), m2 );
					ma.status = "declined";
