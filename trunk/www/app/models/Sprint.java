...
public class Sprint extends SmartModel {
	 ...
	 * number).
	 * @param year: year of the end date.
	 * @param month: month of the end date.
	 * @param day: day of the end date.
	 * @param p: project of this sprint
	 */
	 ...
	 * @param p: project of this sprint
	 */
	public Sprint(int startyear, int startmonth, int startday, int endyear,
			int endmonth, int endday, Project p) {
		this();
		...
		for (int i = 0; i < estimationPointsPerDay.length; i++) {
			for (int j = 0; j < taskss.size(); j++) {
				if (taskss.get(j).estimationPointsPerDay.size() > i) {
					estimationPointsPerDay[i] = estimationPointsPerDay[i]
							+ taskss.get(j).getEffortPerDay(i);
				}
			}
		}
	...
	public static LinkedList<Task> getImpedimentTasks(long Sprint_id) {
		...
		for (int i = 0; i < STasks.size(); i++) {
			...
			if (Current.taskType != null
					&& Current.taskType.name == "Impediment") {
