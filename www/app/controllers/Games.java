...
// GameSession.find("game = ?1 and lastClick >= ?2", game, new Date().getTime()
// - 60000).fetch();
...
// if (round == null) {
// round = new Round(Round.find("byGameAndStory", game,
// game.currentStory).fetch().size()).save();
// round.game = game;
// round.story = game.currentStory;
// round.save();
// }
// List<Round> allRounds = Round.find("game = ?1  order by id desc",
// game).fetch();
//
...
// if (Trick.find("byRoundAndUser", round, Security.getConnected()).first() !=
// null) {
...
// }
//
// int isModerator = response.equals("1") && game.getModerator() == user ? 1 :
// 0;
//
// int rightRound = current == game.getRound() ? 0 : 1;
//
// response += "," + isModerator + "," + rightRound + "," + game.getRound().id +
// "," + roundInfo + ",";
//
// Round round = game.getRound();
...
// response += tricks.get(i).user.name + " " + (roundDone ?
// tricks.get(i).estimate + "" : "done") + "|";
// }
...
// if (gameOver)
// gameOver(game.id);
// game.currentStory = game.stories.get(game.stories.indexOf(game.currentStory)
// + 1);
// game.save();
// }
// Round round = new Round(Round.find("byGameAndStory", game,
// game.currentStory).fetch().size()).save();
// round.game = game;
// round.story = game.currentStory;
// round.save();
