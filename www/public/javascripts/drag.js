...
//	var dina = $('#theLoadedContent').contents().find($('#1_0').contents());
		//$('#theLoadedContent').contents().find('#'+row+'_'+newcol).load('/boards/loadboard1?sprintID='+sid+' '+dina.length);

	//	$('#theLoadedContent').contents().find($('#'+row+'_'+newcol).contents()).load('/boards/loadboard1?sprintID='+sid);
	//	$('#theLoadedContent').contents().find($('#'+row+'_'+oldcol).contents()).load('/boards/loadboard1?sprintID='+sid);

	
//	var children = ($('#theLoadedContent').contents()).getElementById('1_0').childNodes;
...
REDIPS.drag = (function () {
	...
	var	init,					// initialization
		...
		hover_color = '#CCFFCC',
		hover_color1 = '#E7AB83' ;     // (string) hover color
	...
	handler_onmouseup = function (e) {
		...
		// if ordinary element was clicked and left button was released, but element is placed inside unmovable table cell
			...
			/**
			 * @author: joseph hajj
			 * this one assure that if the row !=row to make no changes
			 */
			if (row_source!=row && gup('componentID')==0)
			{	
		...
		/**
		 * 
		 *  depending on the function it will either drag and drop or return
		 * Author:Josephhajj
		 * 
		 */
		...
		$.post('/Tasks/changeTaskStatusHelper' ,			
			{id:gup('sprintID'),columnSequence:cell-1, taskString:obj.id,user_id:selectedUser, cid:gup('componentID')}
		);
		}
	...
	set_trc = function (evt) {
		...
		for (table = 0; table < tables.length; table++) {
			...
			if (tables[table].offset[3] < evt.clientX  &&  evt.clientX < tables[table].offset[1] &&	
				...
				/**
				 * @author: josephhajj
				 * changed the number from 0 to 1 in each to make the headers unreachable
				 */
				if (row < 1 || cell < 1) {
					table = table_old;
					row = row_old;
	...
	calculate_cells = function () {
		var i, j,		// local variables used in loops
			row_offset;	// row box
		// open loop for each HTML table inside id=drag (tables array is initialized in onload event)
	...
	// input parameter is true or false for the first parameter
	// if second parameter isn't defined then all DIV elements will be enabled / disabled
	// second parameter is optional and defines DIV id to enable / disable
	enable_drag = function (enable, div_id) {
		// define local variables
		var i,				// for loop
	...
	return {
		...
		hover_color1			: hover_color1,// (string) hover color
